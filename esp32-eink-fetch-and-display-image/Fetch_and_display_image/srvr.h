...
bool Sleepy__time( int isLongNap )
{
    //SLEEP NOW!!
    // 1 hour: 3600000000
    // 1 min: 60000000
...
void Request__image()
{
  Serial.println("Start image request");
...
bool Srvr__loop()
{
    // Indicate server is online
    digitalWrite(2, HIGH);

    // Go to deep sleep if we've been on for more than a minute
    if (millis() - startTime >= 60000 ) { // i.e. if this thing is on for a minute and it hasn't finished, you can go back to sleep
      Sleepy__time(0);
    }
